...
class KTableActionButtons extends StatelessWidget {
  const KTableActionButtons({
    super.key,
    this.view = false,
    this.edit = true,
    this.delete = true,
      ...
      children: [
        if (view)
          IconButton(
            ...
            icon: const Icon(Iconsax.eye, color: KColors.darkerGrey),
          ),
        if (edit)
          IconButton(
            onPressed: onEditPressed,
            icon: const Icon(Iconsax.pen_add, color: KColors.primary),
          ),
        if (delete)
          IconButton(
            onPressed: onDeletePressed,
            icon: const Icon(Iconsax.trash, color: KColors.error),
          ),
      ],
    );
  }
