...
class KPaginatedDataTable extends StatelessWidget {
  const KPaginatedDataTable({
    super.key,
    ...
    this.dataRowHeight = KSizes.xl * 2,
    ...
    return SizedBox(
      ...
      child: Theme(
        ...
        child: PaginatedDataTable2(
          ...
          headingRowColor: WidgetStateProperty.resolveWith((states) => KColors.primaryBackground),
          empty: KAnimationLoaderWidget(animation: KImages.packageAnimation, text: 'Nothing Found', height: 200, width: 200),
          headingRowDecoration: const BoxDecoration(
            borderRadius: BorderRadius.only(
              topLeft: Radius.circular(KSizes.borderRadiusMd),
              topRight: Radius.circular(KSizes.borderRadiusMd),
            ),
          ),
          sortArrowBuilder: (bool ascending, bool sorted) {
            if (sorted) {
              return Icon(ascending ? Iconsax.arrow_up_3 : Iconsax.arrow_down, size: KSizes.iconSm);
            } else {
              return const Icon(Iconsax.arrow_3, size: KSizes.iconSm);
            }
          },
        ),
      ),
